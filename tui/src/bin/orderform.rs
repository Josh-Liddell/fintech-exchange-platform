...
fn Form<'a>(props: &mut FormProps<'a>, mut hooks: Hooks) -> impl Into<AnyElement<'static>> {
    ...
    let price = hooks.use_state(|| "".to_string());
    let amount = hooks.use_state(|| "".to_string());
        ...
        element! {
            View(
                ...
                View(
                    ...
                    Text(content: "Place an order below", color: Color::White, weight: Weight::Bold)
                    // Text(content: "Press tab to cycle through fields.", color: Color::Grey, align: TextAlign::Center)
                }
                FormField(label: "Account name", value: first_name, has_focus: focus == 0)
                FormField(label: "Price", value: price, has_focus: focus == 1)
                FormField(label: "Amount", value: amount, has_focus: focus == 2)
                // FormField(label: "Life Story", value: life_story, has_focus: focus == 3, multiline: true)
                View(
...
fn main() {
    ...
    if first_name.is_empty() && last_name.is_empty() {
        ...
        println!("Your order has been submitted!");
    }
}
